...
def make_gif(filepath, filename, fps, create_vid, create_bat):
    ...
    # create bat file
    if create_bat:
        LOGGER('Creating .bat file, use it when calculation is interrupted, or to preview output')
    ...
    if create_vid:
        LOGGER('Creating GIF video file - yes, a gif is a video file as well now', filepath,out_filename)
        subprocess.Popen(
                    ...
                    with gr.Column( ):    
                        with gr.Tab("Edit"): 
                            with gr.Column(visible=is_img2img):
                                denoise_sliderstart=gr.Slider(label="denoise_start",minimum=0.0,maximum=1.0,value=0.0)
                                denoise_sliderend=gr.Slider(label="denoise_end",minimum=0.0,maximum=1.0,value=1.0)
                            with gr.Column():
                                cfgscalesliderstart=gr.Slider(label="cfg scale start",minimum=0.0,maximum=30.0,value=5)
                                cfgscale_sliderend=gr.Slider(label="cfg scale end",minimum=0.0,maximum=30.0,value=7) 
                        ...
                        # end ck
                            ...
                            with gr.Column():
                                totaltime=gr.Slider(label="Total Animation Length (s)",minimum=0.0,maximum=600.0,value=30)
                                fps=gr.Slider(label="Framerate",minimum=0.0,maximum=120,value=15,step=1) 
            ...
            """
                              ...
                              i1 = gr.Markdown(
                                            ...
                                            Windows PowerShell:
                                                ...
                                                $env:Path +=";C:\\Users\\ck\\Downloads\\ffmpeg-2022-12-08-git-9ca139b2aa-essentials_build\\bin"
    ...
    def run(self, p,     totaltimeIn, fpsIn, vid_gif, vid_mp4, vid_webm,  denoise_start, denoise_end, cfg_scale_start, cfg_scale_end):
        ...
        for i in range(0,p.n_iter): 
            try:     # If not interrupted, make requested movies. Otherise the bat files exist.
                ...
                LOGGER("Presumably a missing ffmpeg installation is the cause, make sure it is in the search $PATH")
        dickies=[]
